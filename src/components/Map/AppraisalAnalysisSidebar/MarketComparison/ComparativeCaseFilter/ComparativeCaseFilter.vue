<template>
    <div>
        ...
        <div class="filterResults">
            <FilterResultCard
                v-for="(item, index) in filterResults" :key="index"
    ...
    methods: {
        ...
        hightlightMatchSerialsPoints (serials = []) {
            ...
            serials.forEach(serial => {
                const [x, y] = projector(EPSG[3826], EPSG[4326], { x: serial.coordinateY, y: serial.coordinateX })
                const marker = new Marker(latLng(y, x))
                marker.properties = {...serial}
                const content = Object.keys(serial).map(key => (`<div>${key}: ${serial[key]}</div>`))
                marker.bindPopup(`
                <div>
                    ${content.join("")}
                </div>

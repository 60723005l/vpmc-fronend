...
export default {
  ...
  beforeDestroy() {
    if (Global.VPMC.transactionDataStreaming) {
      Global.VPMC.transactionDataStreaming.updateEvent.removeEventListener(
  ...
  methods: {
    async readTransData() {
      ...
      streaming.beforeUpdateEvent.addEventListener(
        this.handleStreamingBeforeUpdate
      );
      streaming.start();
      return streaming;
    },
    handleStreamingBeforeUpdate: async () => {
      await Global.VPMC.asyncViewer;
      Global.VPMC.transactionDataBufferQuery.createBufferGraphic(
        Global.VPMC.viewer.getCenter()
      );
    },
    ...
    addToMap(rawLayers, type) {
      rawLayers.forEach((layer, index) => {
        ...
        Global.VPMC.viewerPromise.execute((viewer) => {
          layerItem.addToMap(viewer).addToControler(Global.VPMC.layerControl);
          layerItem.show = false;
          if (type === "GEOJSON_BUILDING") {
            console.log("================");
            layerItem.content.getLayers().forEach((point) => {
              /**@type {Leaflet.Marker}*/
              const layerPoint = point;
              layerPoint.setIcon(
                new Leaflet.Icon({
                  iconUrl: require("../assets/icon/building.png"),
                  iconSize: [25, 41],
                  iconAnchor: [12, 41],
                  popupAnchor: [1, -34],
                  shadowSize: [41, 41],
  ...
  components: {
    ...
    CustomMapPopup,
    WidgetSidebar: async () =>
      import("../components/Map/WidgetSidebar/WidgetSidebar.vue"),
    AppraisalAnalysisSidebar: async () =>
      import(
        "../components/Map/AppraisalAnalysisSidebar/AppraisalAnalysisSidebar.vue"
      ),
    StatsAndQuoteSidebar: async () =>
      import("../components/Map/StatsAndQuoteSidebar/StatsAndQuoteSidebar.vue"),
    //----------------
    // Layer,
    // Info,
    // Geolocation,
